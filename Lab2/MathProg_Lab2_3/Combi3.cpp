...
namespace combi
{
	...
	a** Initialized(char *set, int size)
	{
		...
		a *q = new a[size];
		std::cout << "Количество множеств: " << getAmount(size) << std::endl;
	...
	void dothis(a *b, a *c, int some, int size, char *set)
	{
		for (int i = 0; i < size; i++)
		{
			...
			{
				if ((i == 0 && b[i].direction == false) || (i == (size - 1) && b[i].direction == true))	//если оно упирается в границы массива
				...
				else
				{
					...
					{																							// если следующее число, в сторону
						dothis(b, c, some - 1, size, set);													// мы двигаемся больше
